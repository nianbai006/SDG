...
def process_item(item):
    """
    Process a single record into a Swift-formatted dict
    Supports two input formats:
    1. legacy format: a single `bboxes` list
    2. new format: separate gemini_misalignment_bboxes + gemini_artifact_bboxes
    """
    ...
    # --- Drop the System message; keep only the User message ---
    # Keeps `content` as List[Dict] so PyArrow does not complain
    messages = [
        {
            "role": "user", 
            "content": [
    ...
    if "gemini_misalignment_bboxes" in item or "gemini_artifact_bboxes" in item:
        # new format: separate misalignment and artifact bboxes
        gt_misalignment_bboxes = item.get("gemini_misalignment_bboxes", [])
        gt_artifact_bboxes = item.get("gemini_artifact_bboxes", [])
        
        # Extract box_2d coordinates
        if gt_misalignment_bboxes and isinstance(gt_misalignment_bboxes[0], dict):
            gt_misalignment_bboxes = [b.get("box_2d", b) for b in gt_misalignment_bboxes]
        if gt_artifact_bboxes and isinstance(gt_artifact_bboxes[0], dict):
            gt_artifact_bboxes = [b.get("box_2d", b) for b in gt_artifact_bboxes]
        
        return {
            "messages": messages,
            "gt_misalignment_bboxes": gt_misalignment_bboxes or [],
            "gt_artifact_bboxes": gt_artifact_bboxes or [],
            "orig_sizes": [w, h],
        }
    else:
        # legacy format: single bboxes list
...
def main():
    parser = argparse.ArgumentParser(description='Preprocess dataset for Swift training')
    parser.add_argument('--input', type=str, required=True,
                        help='Input JSONL file path')
    parser.add_argument('--output', type=str, default=None,
                        help='Output JSONL file path (default: {input_dir}/{input_name}_swift_grpo.jsonl)')
    ...
    if args.output:
        ...
        output_path = os.path.join(os.path.dirname(dataset_path), f"{base_name}_swift_grpo.jsonl")
